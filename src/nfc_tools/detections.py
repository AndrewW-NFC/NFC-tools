...
# ----- Folder-based detection browsing -----

def _is_detection_folder(path: Path) -> bool:
    """Return True if path looks like an NFC Tools night/session folder."""
    if not path.is_dir():
        return False
    ...
    has_audio = audio.exists() and any(audio.glob("*.wav"))
    has_birdnet = any(results.glob("birdnet/*/*.csv"))
    has_nighthawk = any(results.glob("nighthawk/*/*.txt"))

    return bool(has_audio and (has_birdnet or has_nighthawk))


def find_detection_folders(root_path: str | Path, include_subfolders: bool = False) -> list[Path]:
    """Find folders under root_path that look like NFC Tools detection folders."""
    ...
    if not include_subfolders:
        return [root] if _is_detection_folder(root) else []

    matches: list[Path] = []
    for p in [root, *root.rglob("*")]:
        if _is_detection_folder(p):
            matches.append(p)
...
def collect_for_folder(
    ...
    """Collect detections from an explicit NFC Tools night/session folder."""
    nd = Path(folder_path).expanduser().resolve()
    audio = nd / "audio"
    results = nd / "results"

    if not audio.exists() or not results.exists():
        return []
